...
void transfer_complete(HURLPath *path, HURLConnection *conn, size_t content_length, size_t overhead) {
...
int pre_connect(HURLPath *path, HURLConnection *conn) {
	struct tag *t = (struct tag *) path->tag;
	gettimeofday(&t->begin_transfer, NULL);
	return 1; /* Allow connect */
}

void check_response_code(HURLPath *path, HURLConnection *conn, int respcode, char *resptext) {
	struct tag *t = (struct tag *) path->tag;
