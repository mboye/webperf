...
void *hurl_domain_exec(void *domain_ptr)
{
    ...
    while (server != NULL)
    {
        ...
        server->max_connections =
            (unsigned int)roundf((((float)server->nrof_paths
                / (float)domain->nrof_paths)) * (float)domain->max_connections);
        if (server->max_connections <= 0)
        {
            server->max_connections = 1;
        }
        hurl_debug(__func__,
                   "[ %s:%u ] max. connections = %u",
        ...
        for (i = 0; i < (int)server->max_connections; i++)
        {
            ...
            pthread_create(&connection->thread,
            NULL,
                           hurl_connection_exec,
                           connection);
