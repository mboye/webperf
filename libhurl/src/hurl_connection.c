...
int pre_connect(HURLConnection *connection,
                HURLPath *path)
{
    ...
    if (domain->dns_state == DNS_STATE_UNRESOLVED)
    {
        ...
        domain->resolution_time = record_time_msec(&domain->bgof_resolution);
        hurl_debug(__func__, "[ %s:%u ] Domain name resolved in %f ms",
                   domain->domain,
                   server->port,
                   domain->resolution_time);
    }
    ...
    if (domain->dns_state == DNS_STATE_ERROR)
    {
        ...
        {
            manager->hook_transfer_complete(path,
                                            connection,
                                            HURL_XFER_DNS,
                                            0,
                                            0);
...
void *hurl_connection_exec(void *connection_ptr)
{
    ...
    HTTPFeatureSupport feature_persistence = UNKNOWN_SUPPORT;
    HTTPFeatureSupport feature_pipelining = UNKNOWN_SUPPORT;
    unsigned int i;
    HURLPath **queue;
    ...
    domain->manager->connections++;
    hurl_debug(__func__,
               "[ %s:%u ] %d out of %d connections in use.",
               domain->domain,
               server->port,
               manager->connections,
    ...
    HURLPath *path = hurl_server_dequeue(server);
    if (!path)
    {
        /* Nothing for this connection thread to do. */
        hurl_debug(__func__,
                   "[ %s:%u ] No files left to download.",
                   domain->domain,
                   server->port);
    ...
    if (pre_connect(connection, path) != 0)
    {
        pthread_exit(NULL);
    }
    ...
    while (path != NULL)
    {
        ...
        if ((connect_retval = hurl_connect(connection)) != CONNECTION_ERROR)
        {
            ...
            if (domain->manager->feature_pipelining == SUPPORTED
                ...
                && feature_pipelining != UNSUPPORTED)
            {
                ...
                hurl_debug(__func__,
                           "[ %s:%u ] Attempting to pipeline requests.",
                           domain->domain,
                           server->port);
                ...
                while (queue_len < max_pipeline && path != NULL)
                {
                    ...
                    if (manager->hook_send_request == NULL
                        || (manager->hook_send_request != NULL
                            && manager->hook_send_request(path, connection, 1)))
                    {
                        /* Add path to queue. */
                ...
                for (i = 0; i < queue_len; i++)
                {
                    ...
                    if ((response_retval =
                        hurl_connection_response(connection,
                                                 path,
                                                 &buffer,
                                                 &buffer_len,
                                                 &data_len,
                    ...
                    else if (response_retval == 0)
                    {
                        ...
                        if (i < queue_len)
                        {
                            ...
                            hurl_debug(__func__,
                                       "[ %s:%u%.32s ] Connection closed by server.",
                                       domain->domain,
                                       server->port,
                                       path->path);
                        }
            ...
            else
            {
                ...
                if (manager->hook_send_request == NULL
                    || (manager->hook_send_request != NULL
                        && manager->hook_send_request(path, connection, 0)))
                {
                    if (hurl_connection_request(connection, path))
                    {
                        /* Receive HTTP response. */
                        if (hurl_connection_response(connection,
                                                     path,
                                                     &buffer,
                                                     &buffer_len,
                                                     &data_len,
                        ...
                        {
                            gettimeofday(&path->response_received, NULL);
                            if (domain->manager->feature_persistence
                                == UNSUPPORTED
                                || feature_persistence == UNSUPPORTED)
                            {
                                /* Persistent connections not allowed: close connection */
                                hurl_connection_close(connection,
                                                      CONNECTION_STATE_CLOSED);
                                hurl_debug(__func__,
                                           "[ %s:%u ] Connection closed by client.",
                                           domain->domain,
                                           server->port);
                            }
...
int hurl_connect(HURLConnection *connection)
{
    ...
    {
        hurl_debug(__func__,
                   "[ %s:%u ] Previous attempts to connect to this server have failed. Aborting...",
                   domain->domain,
                   server->port);
    ...
    for (i = 0; i < (int)domain->nrof_addresses; i++)
    {
        ...
        else
        {
            ...
            {
                hurl_debug(__func__,
                           "[%s][%u] Failed to create IPv6 socket: %s",
                           domain->domain,
                           server->port,
                           strerror(errno));
                continue;
        ...
        {
            hurl_debug(__func__,
                       "Failed to switch socket to non-blocking mode - %s.",
                       strerror(errno));
            continue;
        }
        ...
        switch ((poll_retval = poll(&poll_sock, 1, (int)timeout)))
        {
            case -1:
                /* Poll failed. */
                hurl_debug(__func__,
                           "[ %s:%u ] Failed to poll with retval %d - %s",
                           domain->domain,
                           server->port,
                           poll_retval,
                           strerror(errno));
            ...
            case 1:
                ...
                if (poll_sock.revents & POLLOUT)
                {
                                   ...
                                   &sock_errno_len) < 0)
                    {
                        ...
                        hurl_debug(__func__,
                                   "[ %s:%u ] Failed to get socket status - %s",
                                   domain->domain,
                                   server->port,
                                   strerror(errno));
                        continue;
        ...
        if (connection->server->tls)
        {
            ...
            struct timeval ssl_begin_connect;
            ...
            if (!SSL_CTX_load_verify_locations(connection->ssl_context,
                                               connection->server->domain->manager->ca_file,
            ...
            while (!ssl_connected)
            {
                switch (poll(&poll_sock,
                             1,
                             (int)server->domain->manager->connect_timeout))
                {
                    case -1:
                        /* Poll failed. */
                        hurl_debug(__func__,
                    ...
                    case 1:
                        ...
                        if ((connect_retval =
                            SSL_connect(connection->ssl_handle)) != 1)
                        {
                            ssl_error = SSL_get_error(connection->ssl_handle,
                                                      connect_retval);
                            ...
                            {
                                ERR_error_string_n((unsigned long)ssl_error,
                                                   ssl_error_str,
                                                   sizeof(ssl_error_str));
                                hurl_debug(__func__,
                                           "SSL connect error: %s",
                                           ssl_error_str);
                                hurl_connection_close(connection,
            ...
            {
                hurl_debug(__func__,
                           "Could not get common name of certificate.");
            }
            ...
            if (!certificate_ok)
            {
                ...
                {
                    hurl_debug(__func__,
                               "Checking certificate for Subject Alternative Names");
            ...
            connection->connect_time_ssl = record_time_msec(&ssl_begin_connect);
        ...
        connection->connect_time = record_time_msec(&connection->begin_connect);
        ...
        if (manager->recv_buffer_len == 0)
        {
            ...
            SOL_SOCKET,
                           SO_RCVBUF,
                           (void *)&manager->recv_buffer_len,
                           &sockopt_len) == 0)
            {
                hurl_debug(__func__,
                           "Receive buffer size: %d",
                           manager->recv_buffer_len);
            }
            else
            {
                hurl_debug(__func__,
                            ...
                            HURLPath *path)
{
    ...
    max_request_len = strlen(path->path) + strlen(path->server->domain->domain)
        + 512;
    ...
    /* File path. */
    request_len += (size_t)snprintf(request + request_len,
                                    max_request_len - request_len,
                                    "GET %s HTTP/1.1\r\n",
                                    path->path);
    ...
    /* End of header. */
    header_len = hurl_header_str(headers,
                                 request + request_len,
                                 max_request_len - request_len);
    ...
    if (header_len > 0)
    {
        ...
        if (hurl_send(connection, request, request_len) != (int)request_len)
        {
            free(request);
            hurl_debug(__func__,
                       "[ %s:%u%.32s ] Failed to send request.",
                       connection->server->domain->domain,
                       connection->server->port,
                       path->path);
            return 0;
    ...
    else
    {
        ...
        hurl_debug(__func__,
                   "ERROR: Request buffer was too small. Could not send request.");
        free(request);
        return 0;
    }
}
                             ...
                             HTTPFeatureSupport *feature_persistence)
{
    ...
    else
    {
        received = *data_len;
        hurl_debug(__func__,
                   "[ %s:%u%.32s ] %ld bytes already in buffer.",
                   connection->server->domain->domain,
                   connection->server->port,
                   path->path,
                   received);
    ...
    while (recv_len > 0)
    {
        if ((recv_len = hurl_recv(connection,
                                  receive_buffer,
            ...
            *(*buffer + *data_len) = '\0';
                ...
                && (eof_header = strstr(*buffer, "\r\n\r\n")) != NULL)
            {
                ...
                while (bgof_line < eof_header)
                {
                    ...
                    if (strncasecmp(line, "http/1.1", strlen("http/1.1")) == 0)
                    {
                        ...
                        *feature_persistence = SUPPORTED;
                        response_code =
                            hurl_parse_response_code(line, &response_code_text);
                        if (manager->hook_response_code)
                        {
                            manager->hook_response_code(path,
                                                        connection,
                                                        response_code,
                                                        response_code_text);
                        }
                        hurl_debug(__func__,
                                   "[ %s:%u%.32s ] Response code: %d %s",
                                   path->server->domain->domain,
                                   connection->server->port,
                                   path->path,
                                   response_code,
                    ...
                    else if (strncasecmp(line, "http/1.0", strlen("http/1.0"))
                        == 0)
                    {
                    ...
                    else if (hurl_header_split_line(line,
                                                    line_len,
                                                    &key,
                                                    &value))
                    {
                        ...
                        {
                            content_type = hurl_allocstrcpy(value,
                                                            strlen(value),
                                                            (size_t)1);
                            hurl_debug(__func__,
                                       "[ %s:%u%.32s ] Content type: %s",
                                       path->server->domain->domain,
                                       connection->server->port,
                                       path->path,
                                       content_type);
                        ...
                        {
                            redirect_location = hurl_allocstrcpy(value,
                                                                 strlen(value),
                                                                 1);
                            hurl_debug(__func__,
                                       "[ %s:%u%.32s ] Redirect location: %s",
                                       path->server->domain->domain,
                                       connection->server->port,
                                       path->path,
                                       redirect_location);
                        ...
                        {
                            transfer_encoding = hurl_allocstrcpy(value,
                                                                 strlen(value),
                                                                 1);
                            hurl_debug(__func__,
                                       "[ %s:%u%.32s ] Transfer encoding: %s",
                                       path->server->domain->domain,
                                       connection->server->port,
                                       path->path,
                                       transfer_encoding);
                            ...
                            else
                            {
                                ...
                                hurl_debug(__func__,
                                           "Bad Transfer-Encoding header. Assuming chunked encoding...");
                                chunked_encoding = 1;
                            }
                        }
                ...
                {
                    manager->hook_header_received(path,
                                                  response_code,
                                                  headers,
                                                  header_len);
                }
                ...
                if (response_code >= 300 && response_code < 400)
                {
                    if (redirect_location != NULL)
                    {
                        ...
                        else if (strncmp("//", redirect_location, 2) == 0)
                        {
                            ...
                            hurl_debug(__func__,
                                       "Absolute protocol-less redirection: '%s'",
                                       redirect_location);
                            redirect_url_len = strlen(redirect_location)
                                + strlen("https://") + 1;
                            redirect_url = malloc(sizeof(char)
                                * redirect_url_len);
                            if (path->server->tls)
                            {
                                snprintf(redirect_url,
                                         redirect_url_len,
                                         "https:%s",
                                         redirect_location);
                            }
                        ...
                        else
                        {
                            ...
                            redirect_url_len = strlen("https://")
                                + strlen(path->server->domain->domain) + 1
                                + strlen(redirect_location) + 1;
                            redirect_url = malloc(sizeof(char)
                                * redirect_url_len);
                        ...
                        if (path->redirect_count < manager->max_redirects)
                        {
                            ...
                            if ((manager->hook_redirect == NULL
                                && manager->follow_redirect)
                                || (manager->hook_redirect != NULL
                                    && manager->hook_redirect(path,
                                                              response_code,
                                                              redirect_url)))
                            {
                                /* Follow redirection by adding a new item to download queue.
                                 * DO NOT ALLOW DUPLICATES! */
                                if ((path_created = hurl_add_url(manager,
                                                                 0,
                                                                 redirect_url,
                                                                 NULL)) == NULL)
                                {
                                    hurl_debug(__func__,
                                               "Failed to add redirect to download queue.");
                                }
                                else
                                {
                                    /* The path was added, to fix tag */
                                    path_created->redirect_count =
                                        path->redirect_count + 1;
                                    path_created->redirector = path;
                                    path->redirectee = path_created;
                                    path_created->tag =
                                        !manager->retag ?
                                            path->tag :
                                            manager->retag(path_created,
                                                           redirect_url);
                    ...
                    {
                        hurl_debug(__func__,
                                   "Redirect detected but location header is missing.");
                    }
                }
            ...
            if (eof_header != NULL)
            {
                if (!chunked_encoding)
                {
                    ...
                    {
                        body_recv_len =
                            header_len + (size_t)recv_len > received ?
                                received - header_len : (size_t)recv_len;
                        ...
                        manager->hook_body_recv(path,
                                                *buffer + header_len
                                                    + hook_recv_body_offset,
                                                body_recv_len);
                ...
                if (!chunked_encoding
                    && received >= header_len + content_len + overhead)
                {
                    transfer_complete = 1;
                    ...
                    hurl_debug(__func__,
                               "[ %s:%u%.32s ] Transfer complete: %d bytes received.",
                               path->server->domain->domain,
                               connection->server->port,
                               path->path,
                               content_len);
                ...
                else if (chunked_encoding)
                {
                    ...
                    {
                        /* Get chunk size */
                        chunk_ptr = *buffer + header_len + content_len
                            + overhead;
                        unprocessed_len = received - header_len - content_len
                            - overhead;
                        k = 0;
                        ...
                        if (unprocessed_len
                            < (size_t)chunk_len_len + 2 + chunk_len)
                        {
                            break;
                        }
                ...
                if (transfer_complete)
                {
                    ...
                    if (*feature_persistence == SUPPORTED)
                    {
                        ...
                        {
                            next_buffer_len = *data_len - header_len
                                - content_len - overhead;
                        }
                        hurl_debug(__func__,
                                   "%u bytes left in buffer.",
                                   next_buffer_len);
                        tmp = malloc(sizeof(char) * (next_buffer_len + 1));
                        memcpy(tmp,
                               *buffer + header_len + content_len + overhead,
                               next_buffer_len);
                ...
                if (chunked_encoding == 1 && response_code > 400
                    && response_code < 300)
                {
                    /* Bad chunked encoding */
                    hurl_debug(__func__,
                               "HTTP redirect: Failed to detect end of chunked encoding.");
                }
            }
        }
        else if (recv_len == 0)
        {
            ...
            hurl_debug(__func__,
                       "[ %s:%u%.32s ] Transfer failed. Connection closed by server.",
                       path->server->domain->domain,
                       connection->server->port,
                       path->path);
            *feature_persistence = UNSUPPORTED;
    ...
    else
    {
        ...
        hurl_debug(__func__,
                   "[ %s:%u%.32s ] Request failed on a reused connection. Retrying...",
                   path->server->domain->domain,
                   connection->server->port,
                   path->path);
        path->state = DOWNLOAD_STATE_PENDING;
