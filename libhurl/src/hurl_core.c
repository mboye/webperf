                       ...
                       void *tag)
{
    ...
    int tls;
    if ((parser_rc = hurl_parse_url(url, &parsed_url))
...
int hurl_exec(HURLManager *manager)
{
    ...
    hurl_debug(__func__,
               "Began execution @ %f",
               timeval_to_msec(&manager->bgof_exec));
    ...
    for (;;)
    {
        ...
        while (domain != NULL)
        {
            if ((nrof_domain_paths =
                hurl_domain_nrof_paths(domain, DOWNLOAD_STATE_PENDING)) > 0)
            {
                domain->max_connections =
                    (unsigned int)roundf((((float)nrof_domain_paths
                        / (float)nrof_paths))
                        * (float)manager->max_connections);
        ...
        while (domain != NULL)
        {
            if (domain->max_connections > 0)
            {
                if (pthread_create(&domain->thread,
                NULL,
                                   hurl_domain_exec,
                                   domain) != 0)
                {
        ...
        hurl_debug(__func__,
                   "Completed: %d",
                   hurl_nrof_paths(manager, DOWNLOAD_STATE_COMPLETED));
        hurl_debug(__func__,
                   "In progress: %d",
                   hurl_nrof_paths(manager, DOWNLOAD_STATE_IN_PROGRESS));
                  ...
                  size_t buffer_len)
{
    ...
    for (;;)
    {
        switch (poll(&poll_sock,
                     1,
                     (int)connection->server->domain->manager->recv_timeout))
        {
            case -1:
                /* Poll failed. */
                hurl_debug(__func__,
            ...
            case 1:
                ...
                if (poll_sock.revents & POLLIN)
                {
                    if (connection->server->tls)
                    {
#ifndef HURL_NO_SSL
                        /* Secure connection. */
                        if ((recv_len = SSL_read(connection->ssl_handle,
                                                 buffer,
                                                 (int)buffer_len)) > 0)
                        {
                        ...
                        {
                            hurl_debug(__func__,
                                       "[ %s:%u ] The SSL connection was closed by the server.",
                                       connection->server->domain->domain,
                                       connection->server->port);
                        ...
                        else
                        {
                            ...
                            if (ssl_error != SSL_ERROR_WANT_READ)
                            {
                                ...
                                {
                                    hurl_debug(__func__,
                                               "[ %s:%u ] SSL read error: %d",
                                               connection->server->domain->domain,
                                               connection->server->port,
                                               ssl_error);
                                    hurl_connection_close(connection,
                                                          CONNECTION_STATE_ERROR); /* TODO: ERROR instead? */
                                    return -1;
                                }
                            }
                        }
                    ...
                    {
                        /* Normal connection. */
                        if ((recv_len = (int)recv(connection->sock,
                                                  buffer,
                                                  buffer_len,
                                                  MSG_NOSIGNAL)) > 0)
                        {
                        ...
                        {
                            hurl_debug(__func__,
                                       "[ %s:%u ] The connection was closed by the server.",
                                       connection->server->domain->domain,
                                       connection->server->port);
                            connection->state = CONNECTION_STATE_CLOSED;
                            return 0;
                  ...
                  size_t buffer_len)
{
    ...
    while (data_sent < buffer_len)
    {
        switch (poll(&poll_sock,
                     1,
                     connection->server->domain->manager->send_timeout))
        {
            case -1:
                /* Poll failed. */
                hurl_debug(__func__,
            ...
            case 1:
                if (poll_sock.revents & POLLOUT)
                {
...
#ifndef HURL_NO_SSL
                        if ((send_len = SSL_write(connection->ssl_handle,
                                                  buffer,
                                                  (int)buffer_len)) > 0)
                        {
                            hurl_debug(__func__,
                    ...
                    else
                    {
                        ...
                        if ((send_len = send(connection->sock,
                                             buffer + data_sent,
                                             buffer_len - data_sent,
                                             MSG_NOSIGNAL)) <= 0)
                        {
                            /* Send failed. */
                            hurl_debug(__func__,
