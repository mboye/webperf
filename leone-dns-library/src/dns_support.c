                         ...
                         char *conf)
{
    ...
    for (i = 0; i < resolv_st.st_size; i++)
    {
        ...
        if (strncasecmp(resolvbuf + i, "nameserver ", strlen("nameserver "))
            == 0)
        {
            i += strlen("nameserver "); /* Move cursor past "nameserver " */
            /* Read until end of line. */
            bgof_line = i;
            for (eof_line = bgof_line;
                eof_line < resolv_st.st_size && resolvbuf[eof_line] != '\n';
                eof_line++)
            {
                /* Do nothing. */
            }
            ...
            else
            {
                nrof_records++;
                /* Set record name. */
                record_name = allocstrcpy(DNS_FAKE_ROOT_SERVER_NAME,
                    ...
                    DNSRecord ***nwp_records)
{
    ...
    if (nrof_nwp_records > 0)
    {
        ...
        if ((tmp = realloc(result, sizeof(DNSRecord *) * nrof_nwp_records))
            != NULL)
        {
            result = tmp;
#ifndef NDEBUG
