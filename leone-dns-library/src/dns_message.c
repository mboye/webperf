...
DNSRecord *dns_record_create(DNSRecordType type,
                             char *name,
                             char *rdata,
...
void dns_fix_pointers(DNSMessage *response,
                      enum network_preference nwp)
{
    DNSRecord *question;
    int i;
    ...
    for (i = 0; i < response->nrof_questions; i++)
    {
        ...
        if (question->type == A || question->type == AAAA)
        {
            ...
            question->answer = dns_message_find_answer(response,
                                                       question->name,
                                                       nwp);
            ...
            if (question->answer != NULL)
            {
                ...
                if (question->answer->type
                    == CNAME && question->answer->answer != NULL)
                {
...
DNSMessage *dns_message_copy(DNSMessage *msg,
                             enum network_preference nwp)
{
    DNSMessage *copy;
    int i;
...
void dns_create_packet(char *qname,
                       DNSRecordType qtype,
 ...
 */
DNSRecord *dns_message_find_answer(DNSMessage *msg,
                                   char *qname,
...
DNSRecord *dns_message_find_rr(DNSMessage *msg,
                               DNSSection section,
                               char *name,
                               enum dns_record_type type)
{
...
DNSRecord *dns_message_find_duplicate(DNSMessage *msg,
                                      DNSRecord *record)
{
    ...
    for (r = 0; r < nrof_records; r++)
    {
        ...
        if (existing_record->type == record->type
            && (strcasecmp(existing_record->name, record->name) == 0))
        {
...
void dns_message_section(DNSMessage *msg,
                         DNSSection section,
                         DNSRecord ***bgof_section,
                         unsigned short *nrof_records)
{
    switch (section)
    {
