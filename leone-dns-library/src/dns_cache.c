...
void dns_cache_node_add_record(DNSCache *cache,
                               DNSMessage *node,
                               DNSRecord *record);
DNSMessage *dns_cache_node_add_child(DNSCache *cache,
                                     DNSMessage *parent,
                                     char *label);

void dns_cache_add_rr(DNSCache *cache,
                      DNSRecord *record)
{
    ...
    {
        log_debug(__func__,
                  "WARNING: Cannot add record with empty RDATA to cache.");
        /* Release cache lock. */
        ...
        if (parent_level == level_prev || i == 0)
        {
            ...
            else
            {
                ...
                for (k = 0; k < (record_level - parent_level); k++)
                {
                    ...
                    parent_node = dns_cache_node_add_child(cache,
                                                           parent_node,
                                                           label);
...
int dns_cache_load(DNSCache **cache,
                   char *conf)
{
    struct stat conf_st;
    char *buf;
    ...
    if (conf != NULL)
    {
        ...
        while (i < conf_st.st_size && nrof_nodes < DNS_MAX_ROOT_SERVERS)
        {
            ...
            while (j < sizeof(type) && i < conf_st.st_size)
            {
                ...
                {
                    log_debug(__func__,
                              "Bad configuration syntax on line %d",
                              line_count);
                    i++;
                    line_count++;
                    continue;
...
DNSCache *dns_cache_init()
{
    ...
    if ((cache->root->children = calloc(DNS_CACHE_NODE_NROF_CHILDREN,
                                        sizeof(DNSMessage *))) == NULL)
    {
        free(cache);
        free(cache->root);
        return NULL;
    ...
    switch (record->section)
    {
        ...
        case AUTHORITIES:
            if (node->nrof_authorities < DNS_MAX_SECTION_RECORDS)
            {
                ...
                {
                    log_debug(__func__,
                              "WARNING: Attempt to add non-NS record (type = %d) to AUTHORITIES section.",
                              record->type);
                }
            }
            else
                                     ...
                                     char *label)
{
    ...
    {
        /* Expand memory. */
        if ((tmp = realloc(parent->children,
                           sizeof(DNSMessage *)
                               * (parent->nrof_children
                                   + DNS_CACHE_NODE_NROF_CHILDREN))) == NULL)
        {
            log_debug(__func__, "Out of memory.");
            exit(EXIT_FAILURE);
        }
...
DNSMessage *dns_cache_find_domain(DNSCache *cache,
                                  char *qname)
{
    DNSMessage *leaf, *root;
    int i, j;
...
DNSMessage *dns_cache_find_best_ns(DNSCache *cache,
                                   char *qname)
{
    DNSMessage *leaf, *root;
    int i, j, x;
