...
void dns_conf_json(DNSResolverState *state,
                   Buffer *buf)
{
    char tmp[1024];
    char timeout[1024];
    ...
    snprintf(tmp,
             sizeof(tmp),
             "{\"timeout\":\"%s\",\"recurse\":%u,\"networkPreference\":%d}",
             timeout,
             state->recurse,
             state->nwp);
...
void dns_cache_json(DNSMessage *root,
                    Buffer *buf)
{
...
void dns_query_json(DNSQuery *query,
                    Buffer *buf)
{
    char tmp[32];
...
void dns_response_json(DNSMessage *response,
                       Buffer *buf)
{
    char tmp[1024];
    char *rdata;
    ...
    snprintf(tmp,
             sizeof(tmp),
             "[%d,%d,%d,%d],",
             response->nrof_questions,
             response->nrof_answers,
             response->nrof_authorities,
             response->nrof_additionals);
    buffer_insert_strlen(buf, tmp);
