                       ...
                       char *qname)
{
    ...
    response->authoritative = dns_message_flag(&flags,
                                               DNS_FLAG_AUTHORITATIVE_ANS,
                                               DNS_FLAG_READ);
    response->truncation = dns_message_flag(&flags,
                                            DNS_FLAG_TRUNCATION,
                                            DNS_FLAG_READ);
    response->recursion_desired = dns_message_flag(&flags,
                                                   DNS_FLAG_RECURSION_DESIRED,
                                                   DNS_FLAG_READ);
    response->recursion_avail = dns_message_flag(&flags,
                                                 DNS_FLAG_RECURSION_AVAIL,
                                                 DNS_FLAG_READ);
    response->response_code = dns_message_flag(&flags,
                                               DNS_FLAG_RESP_CODE,
                                               DNS_FLAG_READ);
    ...
    for (r = 0; r < nrof_records; r++)
    {
        ...
        else if (r < response->nrof_questions + response->nrof_answers)
        {
            ...
            < response->nrof_questions + response->nrof_answers
                + response->nrof_authorities)
        {
            section = AUTHORITIES;
            record_ptr = &response->authorities[r - response->nrof_questions
                - response->nrof_answers];
        }
        else if (r
            < response->nrof_questions + response->nrof_answers
                + response->nrof_authorities + response->nrof_additionals)
        {
            section = ADDITIONALS;
            record_ptr = &response->additionals[r - response->nrof_questions
                - response->nrof_answers - response->nrof_authorities];
        }
        *record_ptr = calloc(1, sizeof(DNSRecord));
        ...
        while ((label_len = dns_parse_rr_label(respbuf,
                                               &cursor,
                                               cursor_max,
                                               namebuf)) > 0)
        {
        ...
        if (label_len == 0)
        {
            ...
            /* Remove trailing dot. */
            if (namebuf->data_len > 0)
            { /* Handle records for root servers. */
            ...
            switch (record->type)
            {
                ...
                case SOA:
                    ...
                    else
                    {
                        ...
                        {
                            log_debug(__func__,
                                      "SOA + 0 answers => Record does not exist.");
                            log_debug(__func__,
                                      "SOA RECORD: %s",
                                      dns_record_rdata_str(record));
                            log_debug(__func__,
                                      "Expected QNAME: '%s' -- Actual QNAME: '%s'",
                                      qname,
    ...
    {
        log_debug(__func__,
                  "WARNING: Parsing ended before end of DNS response (possibly padded).");
    }
#endif
                      ...
                      DNSRecord *record)
{
    ...
    while ((label_len = dns_parse_rr_label(bgof_msg,
                                           cursor,
                                           cursor_max,
                                           databuf)) > 0)
    {
