                          ...
                          int fd_out)
{
    ...
    {
        if ((dns_server = dns_cache_find_rr(test->cache,
    ...
    else if (test->cache != NULL)
    {
        ...
        if (root != NULL)
        {
            ...
            {
                dns_server = dns_cache_find_rr(test->cache,
                                               root->authorities[i]->data,
                                               test->dns_query_type,
                                               ANSWERS,
                                               NULL);
                if (dns_server != NULL)
    ...
    {
        /* DNS OK */
        if (test->elements->http != NULL
            && test->elements->http->download_time == 0)
        {
    ...
    for (cdn = CDN_NONE; cdn < CDN_COUNT; cdn++)
    {
        ...
        for (e = test->elements; e != NULL; e = e->next)
        {
            ...
            if (interrupted && e->http != NULL
                && e->http->result == HURL_XFER_NONE)
            {
                continue;
            }
            ...
            element_cdn =
                e->path->server->domain->tag != NULL ?
                    *((CDNProvider *)e->path->server->domain->tag) : CDN_NONE;
            ...
            if (e->dns != NULL)
            {
                ...
                {
                    log_debug(__func__,
                              "This element triggered DNS resolution and it went OKAY.");
                    /* <AVG LOOKUP TIME FOR SUCCESSFUL LOOKUPS> */
                    avg_dns_ok_time = (avg_dns_ok_time * dns_ok_n
                        + e->dns->exec_time) / (dns_ok_n + 1);
                    lst_dns_ok_time[dns_ok_n] = e->dns->exec_time;

                    /* <AVG ITERATIONS FOR SUCCESSFUL LOOKUPS> */
                    avg_dns_ok_iterations = (avg_dns_ok_iterations * dns_ok_n
                        + e->dns->msg_tx) / (dns_ok_n + 1);
                ...
                {
                    log_debug(__func__,
                              "This element triggered DNS resolution and it did NOT go okay.");

                    /* <AVG LOOKUP TIME FOR FAILED LOOKUPS> */
                    avg_dns_error_time = (avg_dns_error_time * dns_error_n
                        + e->dns->network_time) / (dns_error_n + 1);
                    lst_dns_error_time[dns_error_n] = e->dns->network_time;

                    /* <AVG ITERATIONS FOR FAILED LOOKUPS> */
                    avg_dns_error_iterations = (avg_dns_error_iterations
                        * dns_error_n + e->dns->msg_tx) / (dns_error_n + 1);
            ...
            else if (e->dns_trigger != NULL)
            {
                ...
                log_debug(__func__,
                          "This elements did not trigger DNS resolution, so let's find the element that did.");
            ...
            if (!dns_success)
            {
                ...
                log_debug(__func__,
                          "DNS resolution failed for this element, so HTTP download was never attempted.");
                /* HTTP_NO_IP */
                http_no_ip_n++;
                continue;
            }
            ...
            if (e->http != NULL)
            {
                ...
                if (e->http->download_time > 0)
                {
                    ...
                    {
                        avg_http_ok_connect_time = (avg_http_ok_connect_time
                            * http_ok_connect_n + e->http->connect_time)
                            / (http_ok_connect_n + 1);
                        lst_http_ok_connect_time[http_ok_connect_n] =
                            e->http->connect_time;
                    }
                    ...
                    if (e->http->tls)
                    {
                        ...
                        if (e->http->connect_time > 0)
                        {
                            avg_http_ok_ssl_connect_time =
                                (avg_http_ok_ssl_connect_time
                                    * http_ok_connect_ssl_n
                                    + e->http->connect_time_ssl)
                                    / (http_ok_connect_ssl_n + 1);
                            lst_http_ok_ssl_connect_time[http_ok_connect_ssl_n] =
                                e->http->connect_time_ssl;
                        }

                        /* Download time */
                        avg_https_ok_download_time = (avg_https_ok_download_time
                            * http_ok_ssl_n + e->http->download_time)
                            / (http_ok_ssl_n + 1);
                        lst_http_ok_download_time[http_ok_ssl_n] =
                            e->http->download_time;

                        /* Download size */
                        avg_https_ok_download_size = (avg_https_ok_download_size
                            * http_ok_ssl_n + e->http->download_size)
                            / (http_ok_ssl_n + 1);
                        lst_http_ok_download_size[http_ok_ssl_n] =
                            e->http->download_size;
                    ...
                    else
                    {
                        ...
                        avg_http_ok_download_time = (avg_http_ok_download_time
                            * http_ok_n + e->http->download_time)
                            / (http_ok_n + 1);
                        lst_http_ok_download_time[http_ok_n] =
                            e->http->download_time;

                        /* <AVG FILE SIZE OF COMPLETED DOWNLOADS>  */
                        avg_http_ok_download_size = (avg_http_ok_download_size
                            * http_ok_n + e->http->download_size)
                            / (http_ok_n + 1);
                        lst_http_ok_download_size[http_ok_n] =
                            e->http->download_size;
                    }

                    /* AVG TIME TO FIRST HEADER BYTE */
                    avg_ttfb_header = (avg_ttfb_header * http_ok_n
                        + e->http->bgof_header) / (http_ok_n + 1);
                    lst_ttfb_header[http_ok_n] = e->http->bgof_header;

                    /* AVG TIME TO FIRST BODY BYTE */
                    avg_ttfb_body = (avg_ttfb_body * http_ok_n
                        + e->http->bgof_body) / (http_ok_n + 1);
        ...
        buffer_snprintf(csvbuf,
                        64,
                        "%f;",
                        median_int(lst_dns_ok_iterations, dns_ok_n));
        ...
        buffer_snprintf(csvbuf,
                        64,
                        "%f;",
                        median_int(lst_dns_error_iterations, dns_error_n));
...
double page_load_time(int completeness)
{
    ...
    assert(test->manager->bgof_exec.tv_sec != 0
        && test->manager->bgof_exec.tv_usec != 0);
    for (e = test->elements; e != NULL; e = e->next)
    {
        if (e->http == NULL
            || (e->http != NULL && e->http->result == HURL_XFER_NONE))
        {
