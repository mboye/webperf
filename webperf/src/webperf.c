...
int parse_int(char *value_str,
              int *result,
              int min);
int parse_uint(char *value_str,
               unsigned int *result,
               unsigned int min);
         ...
         char *argv[])
{
        ...
        if (key != NULL)
        {
            ...
            else if (strcasecmp(key, "dns.timeout") == 0)
            {
                ...
                while ((str = strtok_r(s, " ", &timeout_split_ptr)) != NULL)
                {
                    s = NULL;
                    if (!parse_uint(str,
                                    &test->dns_state_template->timeout[j],
                                    1))
                    {
                        break;
                    }
            ...
            {
                if (!parse_uint(value,
                                &test->manager->max_domain_connections,
                                1))
                {
                    break;
                }
            ...
            {
                if (hurl_header_split_line(value,
                                           strlen(value),
                                           &header_key,
                                           &header_value))
                {
            ...
            else if (strcasecmp(key, "stats.http.header") == 0)
            {
                ...
                {
                    log_debug(__func__,
                              "Failed add header key to list: '%s'",
                              line_copy);
                    break;
                }
            }
            ...
            else if (strcasecmp(key, "http.saveBody") == 0)
            {
                if (strlen(value) > 0)
                {
                    ...
                    log_debug(__func__,
                              "Will save downloaded files to '%s'",
                              test->body_path);
                }
                else
                {
            ...
            else if (strcasecmp(key, "http.CAFile") == 0)
            {
                if (strlen(value) > 0)
                {
                    ...
                    test->manager->ca_file = allocstrcpy(value,
                                                         strlen(value),
                                                         1);
                    log_debug(__func__,
                              "OpenSSL will load CAs from '%s'",
                              test->manager->ca_file);
                }
                else
                {
                    break;
            ...
            else if (strcasecmp(key, "test.timeout") == 0)
            {
                ...
                {
                    log_debug(__func__,
                              "Execution timeout set: %u seconds",
                              exec_timeout);
                }
            }
    ...
    {
        log_debug(__func__,
                  "Overriding dns.recurse because dns.resolvconf parameter was specified.");
        test->dns_state_template->recurse = 1;
        log_debug(__func__, "Recursive queries enabled.");
    }

    /* Override output format if no format was specified. */
    if (test->stats.output_format
        == 0|| test->stats.output_format > FORMAT_CSV + FORMAT_JSON)
    {
...
int load_urls(char *file)
{
    ...
    while (j < stat_input.st_size)
    {
        ...
        if ((inputbuf + bgof_line)[0] == '#'
            || strncasecmp("data:", inputbuf + bgof_line, strlen("data:")) == 0)
        {
        ...
        if (line_len > 1)
        {
            ...
            {
                log_debug(__func__,
                          "WARNING: Failed to add elements to download queue.");
