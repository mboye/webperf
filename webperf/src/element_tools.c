                        ...
                        char *filename)
{
    ...
    if (test->stats.output_format & FORMAT_JSON)
    {
        ...
        snprintf(tmp,
                 ...
                 timeout,
                 test->dns_state_template->recurse,
                 nwp_str);
        ...
        printf(
               ...
               WEBPERF_TEST_NAME,
               WEBPERF_TEST_VERSION,
               test->timestamp,
               hostname,
               test->manager->connect_timeout,
               test->manager->send_timeout,
               test->manager->recv_timeout,
               test->http_version,
               test->manager->max_connections,
               test->manager->max_domain_connections,
               test->no_cache,
               test->nwp,
               user_agent,
               timeval_to_msec(&test->manager->bgof_exec),
               test->manager->exec_time,
               interrupted,
               test->tag,
               dns_conf->head);
    ...
    if (stat->http != NULL)
    {
        ...
        if (test->stats.http.redirector && stat->path->redirector != NULL)
        {
            ElementStat *redirector_stat =
                (ElementStat *)stat->path->redirector->tag;
            snprintf(tmp,
                     sizeof(tmp),
                     "\"redirector\":\"%s\",",
                     redirector_stat->url_hash);
            buffer_insert_strlen(json, tmp);
        }

        if (test->stats.http.redirectee && stat->path->redirectee != NULL)
        {
            ElementStat *redirectee_stat =
                (ElementStat *)stat->path->redirectee->tag;
            snprintf(tmp,
                     sizeof(tmp),
                     "\"redirectee\":\"%s\",",
                     redirectee_stat->url_hash);
            buffer_insert_strlen(json, tmp);
        }
                             ...
                             char *destination_url)
{
    ...
    if ((parser_rc = hurl_parse_url(destination_url, &parsed_url))
                      ...
                      HURLPath *path)
{
    ...
    while (redirector != NULL)
    {
        ...
        SHA1_Update(&sha_context,
                    redirector_stat->url_hash,
                    sizeof(redirector_stat->url_hash));
